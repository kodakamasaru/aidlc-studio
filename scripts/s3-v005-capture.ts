/**
 * S3 スクリーンショット生成(v0.0.5)。
 *   bun run scripts/s3-v005-capture.ts   (= package.json の "s3:capture:v005")
 *
 * v0.0.5 は新規画面ゼロ・既存画面の微修正のみ(SCR-02 会話スレッドのバッジ状態整合)。
 * 視覚は v0.0.4 デザインシステム(s3-base.css)を踏襲し、差分(run→review バッジ)だけを
 * 静的 HTML モック(aidlc-docs/v0.0.5/s3/*.html)から撮る。
 *
...
const S3_DIR = resolve(import.meta.dir, "../aidlc-docs/v0.0.5/s3");
