...
## J. 実機ドッグフード feedback 2026-06-15(シナリオ検証ゲートの反転)
出典: ユーザー実機 feedback 2026-06-15(この会話)。「実 AI でシナリオを端まで通す」が構造的に green ゲートに載らず、毎サイクル additive / next-cycle へ逃げる問題。原因は methodology 側の done 定義: 全 AC は決定論層(scripted = 外部 AI のテストダブル)で通すことが必須で、実 AI は「additive(任意)」、終端到達不能な状態は `page.route()` モック注入 + 「正直開示 / carried」で済む。= 一番欲しい「実 AI が多工程シナリオを終端まで自走」が、ハーネスが原理的にゲートにできない唯一のもの。S9/S10 の done 定義を反転して根治する。次サイクル S1 reconcile で必ず拾うこと。

- [ ] **floor / additive の反転**。「実 AI が 1 シナリオを終端状態(質問→回答→`--resume`→次工程 / cycle done)まで完走」をサイクルの**唯一の硬いゲート**にする。scripted orchestrator(決定論テストダブル)と `page.route()` モック注入は**シナリオ AC の pass 証拠として認めない**(回帰用 additive に降格)。遅い・flaky・手動でよい — 決定論にしようとするのをやめる。kit/skills/aidlc-s9-scenario-validation/SKILL.md 本文に焼き込む。関連: §F 方法論 v2(S9/S10) / §I #5(受入ガイド)
- [ ] **skip-on-failure の禁止**。live テストが session_id 未取得時に resume Turn 2 を「report is clear」で skip する経路を除去 → 取れなければ**赤**。「正直開示 / carried / 次サイクルで実機実証」を Phase 完了・done の通過事由にしない。出典: tests/e2e-live/live-us01-us03-us04.test.ts(`SKIP resume sub-path: session_id not captured`)/ s10-acceptance.md O3
- [ ] **REPO_ROOT の env 化**。live テストの絶対パス直書き(`/Users/mac/ghq/.../aidlc-studio`)を環境変数化し、実 AI シナリオを誰の環境でも・CI/ループ内で常設実行可能に(さもないと永遠に「あなたの Mac でしか確認できない=先送り」)。brief Q-02(絶対パス埋め込み禁止)違反の解消も兼ねる。出典: tests/e2e-live/live-run.test.ts / brief Q-02
- [ ] **「実行体が最新か」をループに焼く**。F-9(live がハング/失敗)の最有力原因は `src/` 修正後に `serve` 未再起動で**旧バイナリを実機操作していた**こと。起動フローにビルド世代の可視化(ビルドハッシュ表示 or 起動時自動再ビルド)を入れる。テストでは原理的に防げない層。出典: s10-acceptance.md F-9 対応②

## K. 方法論↔プラットフォーム連動 / seed 跨サイクル(feedback 2026-06-21)
出典: ユーザー実機 feedback 2026-06-21(v0.0.5 S10)。seed の discoverability(README 記載=本サイクルで対応済)を起点に、方法論改善とプラットフォーム改善が構造的に連動しない問題、および seed が跨サイクルを再現できない問題が露呈。詳細は aidlc-docs/v0.0.5/s11-retrospective.md P-ARCH-01 / P-ARCH-02。次サイクル S1 reconcile で必ず拾うこと。

P-ARCH-01 = 方法論(kit)↔ プラットフォーム(コード)の連動機構。AI-DLC のあるべき挙動が ①kit 散文 ②機械ゲート(コード)③データモデル/UI の 3 箇所に別符号化され単一正本が無い。今ある連動は composer の kit 散文注入(散文駆動の挙動のみ)。

**根底の原則(precision-first / 2026-06-21 ユーザー合意)**: 「全部機械ゲート」も「全部散文」も精度を落とす。要件ごとに**一番精度が出る担当**へ振る:
- 作ったか・形式・鮮度・連結 → **機械ゲート(floor)**(決定論・安い・ブレない)。
- 中身が狙いを満たすか(質・説得力・もっともらしさ) → **別 AI evaluator(記録者≠レビュアー)+ 落ちた理由**(ゲートでは精度が出ず偽物が通る)。
- 事業判断・「リリースして大丈夫」の最終確信 → **人間**。
- AI が"良い"の形を理解するための **狙い/役割は平易な intent 文**で残す。**狙いを知らないと AI は無限に検証失敗 or gaming する**ため intent は必須(濃い binding 規則は「届いても守られない」= P36 / 本セッション P-S9-01 で実証、なので最小化)。

- [ ] **(3a) 単一正本の規律【trigger: 今 / 極小】**: 新しい方法論不変条件は step-contracts(型付き単一正本)に宣言し、composer 散文もゲートもそこから導出。新規を散文+別ゲートの二重で足さない(stop digging)。**濃い規則文では書かず、(2) の連結ゲートで構造的に効かせる + 狙いは平易 intent 文で**(AI が良いの形を理解できるように)。
- [ ] **(3b) 橋の型を 1 本で実証【trigger: v0.0.6 早期 / 小】**: `requiresLiveEvidence`(既に step-contracts→散文役割+ゲートの両駆動)に倣い、既存不変条件を 1〜2 個 step-contracts 駆動へ寄せて型を確立。
- [ ] **(3c) 残りを漸進移行【trigger: 随時 / 中・償却】**: 既存の散文+ゲート二重を、触ったとき順次 step-contracts 駆動へ寄せる(big-bang 禁止)。
- [ ] **(1) S12 規範【trigger: v0.0.6 / 極小】**: 機械強制を要する方法論改善は kit 編集と同時にプラットフォーム US を ledger に carried で必ず spawn。
- [ ] **(2) drift 検出 =(3a) の実効化【trigger: v0.0.6 / 小】**: `probe:rules` 拡張で「ルール↔ゲート↔テスト」の連結を機械検査(連結が無ければ赤)。ゲートが見るのは**連結の有無まで**。中身の質は evaluator、最終確信は人間に委ねる(振り分けは上記原則)。
- [ ] **P-ARCH-02: 跨サイクル機能(US-02 台帳 / US-03 reconcile)の project パラメータ化【trigger: 最初の外部PJ導入時 / 小〜中】**。`scripts/reconcile-check.ts` / `scripts/migrate-root-ledger.ts` が `REPO_ROOT = import.meta.dir/..` で studio リポ固定 = 任意 project で動く「プラットフォーム機能」になっておらず studio 専用 CLI(studio dogfood 中は cwd 固定で正しく動くので不要)。対策: reconcile/ledger を repoPath パラメータ化。
- [ ] **seed の跨サイクル fixture【trigger: P-ARCH-02 と同時 / 中】**。現状の seed は 1 project=1 cycle・`ledger.yml` 無しで、跨サイクル機能を seed 上で即確認できない(CLI が studio 固定なので fixture 単独では無意味)。対策: 前サイクル done + `ledger.yml` carried 項目 + 現サイクル S1 の多サイクル fixture + seed materializer を複数 version / ledger.yml 対応に拡張。
