...
サイトを作っても直らないものは、スキル本文 + `kit/rules/aidlc-operating-model.md` を直接修正して根治する。
...
- **AI 開発部 ⇄ 事業部 の責務契約(①内部コード非前提 ②human-gate のみ停止 ③done=納品 ④US+mock が最上位)**: 正本は **`kit/rules/responsibility-contract.md`**(最上位・全工程 binding / 衝突時はこれが勝つ)。ここでは複製しない — **必ず読んで従う**。
