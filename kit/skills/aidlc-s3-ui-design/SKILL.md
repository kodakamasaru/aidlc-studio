...
6. **視覚承認はプラットフォームが描画して取る(脱IDE / 契約①)**。あなたが作る画面 `.html` は **プラットフォームがブラウザでレンダリングしてレビューカードに画像(ギャラリー)で人間に見せる**。**人間にファイル(html)を開かせない**(人間は web のカードしか見ずファイルを開けない)。よって html は「実際にブラウザで描画され人間に画像で見られる」前提で作れ。成果物の `artifacts[]` には scr-NN.html / tokens.html を**必ず含める**(プラットフォームが撮る対象)。`screenshots/*.png`(`bun run s3:capture`)は **S7/S8 が参照する視覚契約**として引き続き生成する(人間レビュー用の描画はプラットフォームが別途行う)。
