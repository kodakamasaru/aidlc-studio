...
| 完了条件 | (1) 全 SCR に html + md + screenshots が揃う / (2) tokens.html が `bun run s3:capture` で tokens.png として撮れている / (3) プラットフォームがレビューカードに画面画像(ギャラリー)を描画し、人間がそれを見て視覚意図を承認(人間にファイルは開かせない / 契約①) / (4) screenshots/ が最新の html と一致(古いキャプチャが残っていない) / (5) **触る US の binding/AC と矛盾しないか逆引き確認済**(矛盾あれば該当 US の確定記録を引用して整合 / operating-model「設計ステップの binding 逆引きゲート」) |
...
1. **視覚方針の擦り合わせ**(`index.md` で集約):
   - **ヒアリング質問は画像なしで自己完結させる(契約①)**: この段階ではまだ画面がプラットフォームに描画されていない(視覚承認は step 6 のギャラリーで取る)。なので**質問カードで「提示中の見た目」「表示中の画面」「下記デザイン」など、人間が今見ていない視覚物を前提に確認を求めるな**。方向性を選ばせるなら各選択肢を**言葉で説明**する(例「暖色ミニマル=食欲を後押しする暖色を差し色に、背景は紙のような温かさ、装飾は最小」)。「提示中のまま進める」のような見えない視覚への参照を選択肢ラベル/ヒントに使わない。実際の見た目の承認は html を作ってから step 6 のレビューで取る。
...
6. **視覚承認はプラットフォームが描画して取る(脱IDE / 契約①)**。あなたが作る画面 `.html` は **プラットフォームがブラウザでレンダリングしてレビューカードに画像(ギャラリー)で人間に見せる**。**人間にファイル(html)を開かせない**(人間は web のカードしか見ずファイルを開けない)。よって html は「実際にブラウザで描画され人間に画像で見られる」前提で作れ。
   - **画面を見せる正しい方法 = `aidlc-result` を出力し `artifacts[]` に scr-NN.html / tokens.html を必ず列挙する**。プラットフォームがそれを撮ってギャラリーにする。これが唯一の経路。
   - **やってはいけない**: 散文に `![説明](…/screenshots/xxx.png)` の Markdown 画像リンクや画像パスを直書きして「これが画面です」とすること。**人間はそのパスを読み込めず画像は出ない**(し、パス露出は契約①違反)。実際に S10 でこの失敗が起き、ギャラリーが出ず生パスだけ表示された。
   - **エンベロープ無しで終わらせない**: 視覚承認を求める工程末尾は、散文だけでなく必ず `aidlc-result`(status=needs_human / artifacts[] に画面ファイル)を出す。これが無いとレビューが legacy のテキスト要約に落ちてギャラリーが描画されない。
   - `screenshots/*.png`(`bun run s3:capture`)は **S7/S8 が参照する視覚契約**として引き続き生成する(人間レビュー用の描画はプラットフォームが artifacts[] の html から別途行う)。
...
## メタ
- 親: UIデザインの一覧
- 視覚 source: この画面のプレビュー(scr-NN / 人間レビュー用)
- スクショ:
