...
| 出力 | `aidlc-docs/{version}/s2.5/` 配下に下記 4 種:<br>① `index.md`(全体方針・Q&A・D)<br>② `tokens.html` + 1 SCR ごとに `scr-NN-{slug}.html`(視覚イメージ source)<br>③ 1 SCR ごとに `scr-NN-{slug}.md`(コンポーネント仕様 / native 固有挙動 / a11y / motion)<br>④ `screenshots/*.png`(`bun run s2.5:capture` で自動生成) |
...
0. **最初に必ず**:
   ...
   - `aidlc-docs/{version}/s2.5/` が既にあれば中身を読む。`index.md` の引き継ぎセクションを最優先で反映。
   - 無ければ `aidlc-docs/{version}/s2.5/` を作り、テンプレで `index.md` を新規作成。
...
### `aidlc-docs/{version}/s2.5/index.md`
...
### `aidlc-docs/{version}/s2.5/tokens.html` の骨子
...
### `aidlc-docs/{version}/s2.5/scr-NN-{slug}.html` の骨子
...
### `aidlc-docs/{version}/s2.5/scr-NN-{slug}.md`(コンポーネント仕様)
