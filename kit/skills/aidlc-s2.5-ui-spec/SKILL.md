---
name: aidlc-s2.5-ui-spec
description: AI-DLC S2.5。S2 で確定した情報構造に対し、視覚意図(色 / タイポ / 余白 / 状態 / 階層 / 密度)を HTML イメージで作る。**HTML は実装テンプレートではなくイメージのリファレンス**。S6/S7 は screenshots/*.png + *.md だけ参照し、HTML/CSS コード構造は参照禁止。ユーザーが「UI 詰めたい」「見た目決めたい」「デザイン固めたい」と言ったとき、または s2/ が確定して s2.5/ がまだ無いときに呼ぶ。
---

# AI-DLC S2.5: UI 設計確定(視覚意図のイメージ作り)
...
**最重要**: S2.5 の HTML は **視覚意図のリファレンス**であって、**コード移植元ではない**。S6/S7 は HTML を絶対に Read せず、HTML から生成された `screenshots/*.png` と コンポーネント仕様 `*.md` だけを参照する。

なぜそんな運用にするか — 過去 v0.0.1 で S6/S7 が UI を「実装しやすさ」「実機確認以外だから」を理由に独断で質素化した事故が起きたため。S2.5 を新設して視覚契約を先に固め、Construction 工程が **視覚アウトカム(色 / 余白 / 階層)を契約として受け取る** が、**実装テクニック(HTML タグ / CSS / Tailwind)は参照させない** という構造で防ぐ。

## いまどこにいるか

- フェーズ: **Inception** の S2.5
- 前: `aidlc-docs/{vX}/s2/`(画面モック + 情報構造の Biz 合意済)
- 後: S3(Unit of Work)/ S4 / S5 と並行可能。**S6 着手時点で S2.5 が `確定` していること**が前提
- 関連参照: `[[aidlc-s2.5-image-only-contract]]` / `[[feedback-no-scope-cut-by-construction]]`

## 入出力と完了条件

| 項目 | 内容 |
|------|------|
| 入力 | `aidlc-docs/{vX}/s2/index.md` と `s2/scr-NN-*.md` 群(全 SCR が `確定`) |
| 出力 | `aidlc-docs/{vX}/s2.5/` 配下に下記 4 種:<br>① `index.md`(全体方針・Q&A・D)<br>② `tokens.html` + 1 SCR ごとに `scr-NN-{slug}.html`(視覚イメージ source)<br>③ 1 SCR ごとに `scr-NN-{slug}.md`(コンポーネント仕様 / native 固有挙動 / a11y / motion)<br>④ `screenshots/*.png`(`bun run s2.5:capture` で自動生成) |
| 完了条件 | (1) 全 SCR に html + md + screenshots が揃う / (2) tokens.html が `bun run s2.5:capture` で tokens.png として撮れている / (3) ユーザーがブラウザで html を見て視覚意図を承認 / (4) screenshots/ が最新の html と一致(古いキャプチャが残っていない) |

## 進め方

0. **最初に必ず**:
   - `aidlc-docs/{vX}/s2/index.md` と `s2/scr-NN-*.md` を全件読む。各 SCR の「目的・主要要素・対応 US」を把握。
   - `aidlc-docs/{vX}/s2.5/` が既にあれば中身を読む。`index.md` の引き継ぎセクションを最優先で反映。
   - 無ければ `aidlc-docs/{vX}/s2.5/` を作り、テンプレで `index.md` を新規作成。
...
4. **`bun run s2.5:capture` でスクリーンショット生成**:
   - `scripts/s2.5-capture.ts`(Playwright headless Chromium)が `.html` の `data-state` section ごとに `screenshots/{scr-name}.{state}.png` を書き出す
...
6. Biz レビューはブラウザで html を開いてもらって視覚承認を取る(screenshots だけだと細部が潰れるため source 確認は人間 OK / S6/S7 だけ html を見ない)。

7. 完了条件 4 つが全て埋まったら `index.md` のステータスを `確定` にして S3/S5/S6 を案内。

## 成果物テンプレート

### `aidlc-docs/{vX}/s2.5/index.md`
...
## 次工程への引き継ぎ
- S6/S7 が参照すべき screenshots と md の対応表:
...
### `aidlc-docs/{vX}/s2.5/tokens.html` の骨子
...
<html lang="ja">
  <head>
    <meta charset="utf-8">
    <title>tokens — S2.5 視覚カタログ</title>
...
### `aidlc-docs/{vX}/s2.5/scr-NN-{slug}.html` の骨子
...
### `aidlc-docs/{vX}/s2.5/scr-NN-{slug}.md`(コンポーネント仕様)
...
- 視覚 source: [scr-NN-{slug}.html](./scr-NN-{slug}.html)(人間レビュー用 / S6/S7 は Read 禁止)
- スクショ:
...
- S2 を読まずに画面イメージを描く
- HTML を「実装が楽になる構造」で書く(視覚アウトカムの再現性が最優先 / DOM 構造は S6/S7 が参照しないので最適化しない)
- 配色 / 階層 / 余白を tokens なしのインライン値で書く(変更時に追えなくなる)
- 状態(loading / empty / error / disabled / focused)を default だけ描いて省略する → 状態網羅性が S6/S7 実装契約の質を決める
- 1 ファイルに 10 画面まとめて書く(画面 = 1 HTML を守る)
- screenshots/ を gitignore する(差分レビュー対象に含める)
- 古い screenshots を放置したまま `.html` を変更する → `bun run s2.5:capture` 前に全削除
- **このスキルから S6/S7 に「HTML を Read していい」と示唆する**(契約の根幹を壊す)

## やり直しの判断

- **S2 に戻る**: 情報構造そのものに不足 / 矛盾が見つかった(画面要素の追加・統合・削除が必要)
- **S1 に戻る**: そもそも US の網羅が足りない / 認識ずれが視覚化して初めて見えた(まれだが起きる)
- **このまま S2.5 で書き直す**: 視覚方針の方向性が違った(スタイル選定 / カラー方針の練り直し)
