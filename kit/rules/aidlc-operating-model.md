...
7. **視覚成果物が出ている(S7 で画面に触れた Phase は必須)**: Phase 完了時に `verify-ui`(Expo Web + Playwright)で **screenshot を自動生成し成果物として提示**する。人間に求めるレビューは **実機確認と視覚レビューのみ**。**AI が人間に「コードを見てレビューして」と求めることは禁止**(コード品質は 7 reviewer が担保する責務 / 人間の時間は実機+視覚に限定)。
...
3. **サイクル戻り時の引き継ぎを書く** — `## 前サイクルからの引き継ぎ`(手戻り時に追加)+ S7 の `## 次サイクルへの引き継ぎ`(必須)。さらに **確定した `### D-NN` / 次サイクルに渡す項目は `aidlc-docs/{vX}/ledger.yml` に `state: carried|done|dropped` で台帳化**(散文だけに頼らない)。次サイクル S1 は **未 reconcile 項目(carried 放置)をゼロにするまで `確定` にできない**。詳細・スキーマは [ledger.md](ledger.md)。
...
- AI 提案を人間レビューなしで採用する
- **AI が人間にコードレビューを求める**(人間の役割は実機確認+視覚レビューのみ / コード品質は 7 reviewer 自動 pipeline の責務 / Phase 完了時は screenshot 等の視覚成果物を出す)
