...
/**
 * US-06: open Step 設定, give S1 a verification observation, save. A step that
...
async function optInGenEval(page: Page, shotName: string | null): Promise<void> {
  ...
  await expect(page.getByRole("heading", { name: "Step 設定" })).toBeVisible();
  ...
  await expect(s1Form.getByText("単一 Run")).toBeVisible();

  await page
    .locator("#S1-obs")
    .fill("要件1: 一覧が表示される\n要件2: 空状態が表示される");
  // The badge flips to gen→gate→eval as soon as observations are present.
  await expect(
    s1Form.locator(".step-card__badge", { hasText: "gen→gate→eval" }),
  ).toBeVisible();

  await s1Form.getByRole("button", { name: "契約を保存" }).click();
  ...
  await expect(
    page
      ...
      .locator(".step-card__badge", { hasText: "gen→gate→eval" }),
  ).toBeVisible();
}
...
async function startS1(page: Page, goal: string): Promise<void> {
  await page
    ...
    .first()
    .click();
  await page.getByLabel("Cycle 名(ゴール)").fill(goal);
...
test("complete: step opt-in runs gen→gate→eval and renders a 2/2 completeness review", async ({
  ...
  await expect(
    page.getByRole("heading", { name: /成果の確定レビュー/ }),
  ).toBeVisible();
  const completeness = page.getByRole("region", { name: "完全性チェック" });
  await expect(completeness).toBeVisible();
  await expect(completeness.getByText("2/2 要件 対応")).toBeVisible();
...
test("descope: a gap surfaces a reasoned 見送り判断 and routes to the backlog", async ({
  ...
  const choices = page.getByRole("group", { name: "見送り判断の選択肢" });
  await expect(choices.getByRole("button", { name: /つくる/ })).toBeVisible();
  await expect(choices.getByRole("button", { name: /見送る/ })).toBeVisible();
  await expect(choices.getByRole("button", { name: /後回し/ })).toBeVisible();
  await expect(
    choices.getByRole("button", { name: /前のステップからやり直す/ }),
  ).toBeVisible();
  await shotS9(page, "us-03.descope-decision.png");

  // Decide 見送る (descope → backlog). The app returns to the cycle screen and
  // the descope card is consumed from the Inbox (the requirement now lives in the
  // backlog, not silently lost).
  await choices.getByRole("button", { name: /見送る/ }).click();
  ...
  await page.goto(`${DESCOPE}/inbox`);
  await expect(
    page.getByRole("listitem").filter({ hasText: "見送り判断" }),
  ).toHaveCount(0);
});
