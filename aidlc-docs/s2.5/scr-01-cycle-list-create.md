...
### D-02 — 作成フォームから「対象リポ」「初期 Task」を削除(ユーザー指摘で修正)
- **理由**: ユーザーレビューで「リポ=PJ なので選ぶ意味がない」「初期 Task の意味が不明」と指摘。確認の結果、repo-switch(US-25)も Backlog/Task も **v0.0.x** で MVP に無く、入口フォームが v0.0.x 概念を先取りしていた。MVP は「Cycle 名 + 概要(任意)」に単純化。単一 Task 既定は背景生成。S2 SCR-01(情報構造)へ手戻り反映済、落としは BACKLOG.md へ。
- **判断**(ユーザー記入): 承認 | 上書き | 保留
- **上書き内容**(上書き時のみ):
