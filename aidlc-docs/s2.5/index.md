...
### D-06 — SCR-01 作成フォームを単純化(ユーザー指摘で S2 へ手戻り)
- **理由**: レビューで「対象リポは PJ=リポなので選ぶ意味がない」「初期 Task が意味不明」と指摘。repo-switch(US-25)も Backlog/Task も v0.0.x で MVP に存在しないのに入口フォームが先取りしていた。MVP フォームを「Cycle 名 + 概要(任意)」に縮小し、単一 Task 既定は背景生成。S2 SCR-01(情報構造)へ反映 + BACKLOG.md 台帳化。
- **判断**(ユーザー記入): 承認 | 上書き | 保留
- **上書き内容**(上書き時のみ):
