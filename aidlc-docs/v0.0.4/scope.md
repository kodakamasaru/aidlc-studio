...
| g | 可変ステップ数の進捗表示 | 2026-06-13 S3 レビュー指摘(ユーザー) | US-07 | サイクル画面のプログレス([PhasePipeline.tsx](../../web/src/features/cycle-detail/PhasePipeline.tsx))が、ステップ数可変(任意ステップの有無)でも破綻せず一目で現在地が読める。12 ステップを 5 PhaseGroup に束ねて横溢れ/窮屈を防ぐ |

> US の最終確定(本数・粒度・分割)は S1 Phase B で行う。上表は scope の起点であり目標本数ではない(US 数は出力であって目標ではない)。
>
> **項目 g は 2026-06-13 の S3 レビューで追加**(ユーザー指摘「可変ステップでプログレスバーの見た目は考慮しているか」)。S3 進行中の差し込みのため S1(US-07)→ S2(SCR-05)→ S3 を逆算で補完。主軸(S1 を web で)は不変、g は進捗可視化の独立スライス。
