...
### Q-01 — v0.0.2 スコープはこの 4 US(US-07/13/18/27)でよいか?
- 観点: 品質ハーネス(A-H)の内部実装は US-07/13 に吸収、リッチ描画(K)は US-18、Step カスタム(I)は US-27。新規 US を立てるべきものがあるか。例: descope 問い合わせ用の専用 US / evaluator 状態の可視化 US。
- **回答**(ユーザー記入):
  >
- **確定**(AI 記入):
  >

### Q-02 — carried 4件(S7-C1/C2/C3/C4)を全部 v0.0.3+ に先送りしてよいか?
- 観点: S7-C4(relaunchPhase)は backrack 後の再実行に直結する機能。v0.0.2 の evaluator retry(gen→eval ループ)と相乗りする可能性がある。S7-C4 も v0.0.2 に入れるべきか?
- **回答**(ユーザー記入):
  >
- **確定**(AI 記入):
  >

### Q-03 — 品質ハーネスの新概念(BriefIn-Out / Profile / evaluator)をユーザー可視にするか?
- 観点: 現在の提案では BriefIn-Out と Profile レジストリは内部実装(ユーザーには見えない)。ユーザーが「どの requirements が covered か」を確認できるように completeness table を US-18 に入れたが、他にもユーザーに見せるべき概念があるか?
- **回答**(ユーザー記入):
  >
- **確定**(AI 記入):
  >
